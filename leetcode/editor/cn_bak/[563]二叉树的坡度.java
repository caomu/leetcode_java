...
class Solution {
    public int findTilt(TreeNode root) {
        ...
        return Math.abs(this.sum(root.left) - this.sum(root.right)) + this.findTilt(root.left) +
    ...
    private int sum(TreeNode node) {
        ...
        int leftVal = node.left == null ? 0 : this.sum(node.left);
        int rightVal = node.right == null ? 0 : this.sum(node.right);
