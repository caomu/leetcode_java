...
public class Solution {
	public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
		...
		while (true) {
			if ((!firstRoundA && headA.next == null) || (!firstRoundB && headB.next == null)) {
				return null;
			}
			if (firstRoundA && headA.next == null) {
				firstRoundA = false;
				headA = startB;
			} else {
				headA = headA.next;
			}
			if (firstRoundB && headB.next == null) {
				firstRoundB = false;
				headB = startA;
			} else {
				headB = headB.next;
			}
			if (!firstRoundA && !firstRoundB && headA == headB) {
				return headA;
			}
		}
	}
