...
public class _740_DeleteAndEarn {
    ...
    class Solution {
        public int deleteAndEarn(int[] nums) {
            Map<Integer, Integer> numPoints = new TreeMap<>();
            var maxNum = Integer.MIN_VALUE;
            for (var num : nums) {
                maxNum = Math.max(maxNum, num);
                numPoints.put(num, numPoints.getOrDefault(num, 0) + num);
            }
            return this.deleteAndEarn(new ArrayList<>(numPoints.keySet()), numPoints, maxNum, new int[maxNum]);
        }
