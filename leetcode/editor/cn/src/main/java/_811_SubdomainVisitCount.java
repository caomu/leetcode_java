...
public class _811_SubdomainVisitCount {
    public static void main(String[] args) {
        Solution solution = new _811_SubdomainVisitCount().new Solution();
        ...
        public List<String> subdomainVisits(String[] cpdomains) {
            ...
            for (String cpdomain : cpdomains) {
                ...
                if (domainSplit.length == 2) {
                    domainFrequency.put(domainSplit[1], domainFrequency.getOrDefault(domainSplit[1], 0) + frequency);
                } else if (domainSplit.length == 3) {
                    domainFrequency.put(domainSplit[2], domainFrequency.getOrDefault(domainSplit[2], 0) + frequency);
                    String subDomain = domainSplit[1] + '.' + domainSplit[2];
                    domainFrequency.put(subDomain, domainFrequency.getOrDefault(subDomain, 0) + frequency);
                }
                domainFrequency.put(split[1], domainFrequency.getOrDefault(split[1], 0) + frequency);
            }
            return domainFrequency.entrySet().stream().map(entry -> entry.getValue() + " " +
