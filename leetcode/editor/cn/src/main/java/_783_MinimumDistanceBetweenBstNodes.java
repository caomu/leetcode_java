...
public class _783_MinimumDistanceBetweenBstNodes {
    ...
    class Solution {
        public int minDiffInBST(TreeNode root) {
            return root == null ? Integer.MAX_VALUE : (Math.min(Math.min(
                    root.left == null ? Integer.MAX_VALUE : root.val - this.findRightChild(root.left),
        ...
        private int findLeftChild(TreeNode node) {
            return node.left == null ? node.val : this.findLeftChild(node.left);
        }

        private int findRightChild(TreeNode node) {
            return node.right == null ? node.val : this.findRightChild(node.right);
        }
    }
